...
export const listAllOrders = [
  async (
    req: Request,
    res: Response<
      Order[] | ValidationErrorResponse | ErrorResponse | ResponseWithMessage
    >
  ) => {
    try {
      const user: any = getPayloadFromAccessToken(
        req.headers.authorization?.split(" ")[1] || ""
      );
      const order: Order[] = await findAllOrders({
...
export const createOrder = [
  async (
    ...
    try {
      ...
      const ticket: any = await findAllTickets({
        _id: new mongoose.Types.ObjectId(tickets),
      });
